...
/** A batch of monetary flows from one partition to another.
  *
...
object BatchedFlow:
  ...
  /** N:M flow — amounts indexed by sender. Each sender pays their assigned target.
    *
    * amounts(42) = how much sender #42 pays. targetIndices(42) = which receiver gets it. amounts.length == partitionSize(from).
    */
  case class Scatter(
  ...
  /** 1:N flow — amounts indexed by receiver. Single sender pays all.
    ...
    * partitionSize(to) if sparse). totalDebit is aggregated in one shot — avoids cache thrashing on fromStore.
    */
