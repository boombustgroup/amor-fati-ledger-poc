...
class GenericContractSpec extends AnyFlatSpec with Matchers:
  ...
  it should "reject currency mismatch, permissions, and bounds" in {
    ...
    val receiveDenied = LedgerTopology.validate(Map(A -> AccountMetadata(X), B -> AccountMetadata(X, canCredit = false))).toOption.get
    TransferExecutor.execute(receiveDenied, Map(A -> 50L, B -> 0L), transfer, 1L).isLeft shouldBe true
  ...
  it should "execute sequences atomically and preserve evidence totals" in {
    ...
    TransferExecutor
      .executeSequence(topology(), initial, Vector(Transfer(A, B, 150L, M, P), Transfer(B, A, 1L, M, P)), 9L)
